...
class InteresseNoHorarioController < ApplicationController
  ...
  def index
       ...
       order(:dia_da_semana).where("coalesce(matriculas.data_matricula,matriculas.data_inicio) <= current_date and (matriculas.data_fim is null or matriculas.data_fim > ?)", Time.now).group_by{|h| h.dia_da_semana}
