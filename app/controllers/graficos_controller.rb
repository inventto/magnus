class GraficosController < ApplicationController
  def index
    ...
    @desistentes_por_mes_e_ano = @desistentes.group_by{|m| "#{m.data_fim.year}/#{m.data_fim.month}"}

    @matriculas_por_mes_e_ano = @matriculas.group_by{|m| "#{m.data_inicio.year}/#{m.data_inicio.month}"}
