...
class ClientesInativosController < ApplicationController
  ...
  def filtrar
    ...
    if dias_da_semana = params["dias_da_semana"] and not dias_da_semana.blank?
      ...
      mats = Matricula.select("count(dia_da_semana), matricula_id").joins(:horario_de_aula).group(:matricula_id).having("count(dia_da_semana) = ?", dias.length)
      if not mats.blank?
        matriculas_id = []
      ...
      @matriculas = @matriculas.where("id IN (?)", matriculas_id)
      all_blank = false
