...
class RegistroPresencaController < ApplicationController
  ...
  def gerar_falta eh_feriado
    ...
    horarios = horarios.where("data_inicio <= ? and (data_fim is null or data_fim >= ? and (inativo_ate is null and inativo_desde is null and inativo_desde > inativo_ate))", today.to_date, today.to_date)
